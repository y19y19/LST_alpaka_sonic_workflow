...
parser = getParser()
parser.add_argument("--config", default="step3_RAW2DIGI_RECO_VALIDATION_DQM_PU", type=str, help="cmsDriver-generated config to import")
...
if options.sonic:
    ...
    modifier_names = ["enableSonicTriton", "trackingLSTSonicTriton"]
...
'''
import re
for name, obj in process.__dict__.items():
    try:
        dump = obj.dumpPython()
        if 'lst' in dump.lower():
            print(f"YY: found LST in: {name}")
    except:
        pass
'''
